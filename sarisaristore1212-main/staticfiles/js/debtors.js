...
function getDebtorStyles() {
  return `
    <style>
      .debtors-summary-cards { display: grid; grid-template-columns: repeat(3, 1fr); gap: 16px; margin-bottom: 40px; }
...
try { await DB.addSale({ total: amount, profit: totalProfit, payment_method: 'credit-paid', customer_name: customerName, items: saleItems }); }    catch (saleError) { console.error('Error creating sale record:', saleError); }
